...
@section('title', __('app.sub_accounts.add'))
@if(! $isUserPanel)
@section('page-title', __('app.sub_accounts.add'))
@endif
...
@section('content')
    @if($isUserPanel)
        ...
        <div class="ud-dashboard ud-fade-in sub-account-form">
            <header class="mb-4">
                <h1 class="ud-page-title">{{ __('app.sub_accounts.add') }}</h1>
                <p class="ud-page-sub mb-0">{{ __('app.sub_accounts.create_hint') }}</p>
            </header>
            ...
            <form method="POST" action="{{ route('user.sub-accounts.store') }}">
                @csrf
                ...
                <div class="d-flex flex-wrap gap-2">
                    ...
                    <button type="submit" class="ud-btn ud-btn--primary">{{ __('app.common.save') }}</button>
                </div>
            </form>
        </div>
    @else
        <x-admin.form-shell
            :action="route($panel . '.sub-accounts.store')"
