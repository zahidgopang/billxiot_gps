...
(function (global) {
    ...
    class TraccarUi {
        ...
        openVehiclePopup(id) {
            ...
            const lat = st.renderPos?.lat ?? v.lat;
            const lng = st.renderPos?.lng ?? v.lng;
            ...
            if (st.marker) {
                if (typeof st.marker.setZIndex === 'function') {
                    st.marker.setZIndex(4000 + numId);
                }
                st.marker.setMap(this.markerShouldShowOnMap(numId) ? this.map : null);
            }
