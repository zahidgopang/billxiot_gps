#!/usr/bin/env python3
"""Compose BillXiot GPS horizontal logos from the BILLX wordmark + GPS suffix."""
...
def sync_mobile(light: Image.Image, dark: Image.Image, wordmark: Image.Image) -> None:
    ...
    fav = compose_favicon(wordmark, 1024)
    fav.save(icon_dir / "icon-1024.png", optimize=True)
    compose_favicon(wordmark, 512).save(icon_dir / "icon-512.png", optimize=True)
    compose_favicon(wordmark, 512).save(MOBILE / "mobile" / "app-icon.png", optimize=True)
